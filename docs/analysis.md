...
│       ├── azure-terraform-deploy.yml     # [Reusable workflow - parent]
...
|----------|------|---------|----------|
| **Azure Terraform Deploy** | Reusable (Parent) | Standardized Terraform validation, security scanning, planning, and deployment to Azure with OIDC authentication and approval gates. | `.github/workflows/azure-terraform-deploy.yml` |

**Key Features:**
- Four-job pipeline: Validate → Security Scan → Plan → Apply
- OIDC-based authentication with separate Plan (Reader) and Apply (Owner) managed identities
- Checkov security scanning with SARIF upload
- Terraform plan artifacts and PR comments
- Environment-based approval for apply job
...
| `.github/workflows/azure-terraform-deploy.yml` | `nathlan/shared-assets` | `<YOUR_GITHUB_ORG>/shared-assets` | Reusable workflow repository reference |
...
|--------|-------|--------------|-------|
| `GH_AW_AGENT_TOKEN` | Agentic workflows (`alz-vending-dispatcher.md`, `github-config-dispatcher.md`) | Agent assignment, cross-repo issue creation | Repository-level |
| `AZURE_CLIENT_ID_TFSTATE` | `azure-terraform-deploy.yml` reusable workflow | Terraform backend state operations | Passed by consuming repositories |
| `AZURE_CLIENT_ID_PLAN` | `azure-terraform-deploy.yml` reusable workflow | Terraform plan (read-only) | Passed by consuming repositories |
| `AZURE_CLIENT_ID_APPLY` | `azure-terraform-deploy.yml` reusable workflow | Terraform apply (owner role) | Passed by consuming repositories |
| `AZURE_TENANT_ID` | `azure-terraform-deploy.yml` reusable workflow | OIDC authentication | Passed by consuming repositories |
| `AZURE_SUBSCRIPTION_ID` | `azure-terraform-deploy.yml` reusable workflow | Azure SDK initialization | Passed by consuming repositories |

### Variables Referenced

None found at the shared-assets repository level. Consuming repositories will define their own environment-specific variables.
...
- **Called by:** Consuming repositories via `uses: <YOUR_GITHUB_ORG>/shared-assets/.github/workflows/azure-terraform-deploy.yml@<ref>`
- **Parameters:**
  - `environment` (required) — Deployment environment identifier
  - `terraform-version` (optional, default: `latest`) — Terraform version
  - `working-directory` (optional, default: `terraform`) — Terraform root directory
...
- ✅ **2 instruction files** (CI/CD best practices, Markdown standards)
